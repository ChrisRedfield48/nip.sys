...
```
  ...
  ◉  авто-эко-режим — при просадке FPS сам снижает нагрузку
...
- авто-качество: `applyQuality()` при просадке FPS снижает нагрузку (DPR → 1, выключает туманность и размытие навбара, потолок 30 fps). Пороги срабатывания — в `frame()`: ниже `50` и ниже `40` fps
