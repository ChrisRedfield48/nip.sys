(() => {
  ...
  function buildNet() {
    ...
    for (let li = 0; li < L - 1; li++) {
      ...
      for (let a = a0; a < a1; a++)
        for (let b = a1; b < b1; b++) {
          if (small && Math.random() < 0.3) continue;
          edges.push({ a, b });
        }
      off += sizes[li];
    }
  ...
  function project() {
    ...
    for (const n of nodes) {
      ...
      n.sc = sc;
      n.da = Math.max(0.25, Math.min(1, 0.55 + z2 * 0.5));
    }
  }
  function spawnForward() {
    ...
    for (const n of first) {
      ...
      for (const ei of n.out)
        if (Math.random() < 0.8 && signals.length < (small ? 50 : 84))
          signals.push({
            e: ei,
            t: 0,
            sp: 0.45 + Math.random() * 0.35,
            dir: 1,
            gen: 0,
            col: C.mint,
          });
  ...
  function spawnBack() {
    ...
    for (const n of nodes) {
      if (n.layer !== lastL) continue;
      if (Math.random() < 0.7) {
        n.act = 1;
        for (const ei of n.inn)
          if (Math.random() < 0.55 && signals.length < (small ? 50 : 84))
            signals.push({
              e: ei,
              t: 0,
              sp: 0.5 + Math.random() * 0.3,
              dir: -1,
              gen: 0,
              col: C.purple,
            });
  ...
  function burst(px, py) {
    ...
    if (!best) return;
    best.act = 1.4;
    for (const ei of best.out)
      if (signals.length < (small ? 50 : 84))
        signals.push({
          e: ei,
          t: 0,
    ...
    for (const n of nodes) {
      const dx = n.sx - px,
        dy = n.sy - py;
      if (dx * dx + dy * dy < (small ? 70 : 100) ** 2)
        n.act = Math.min(1.4, n.act + 0.7);
    }
  }
  function step(dt) {
    const lastL = nodes[nodes.length - 1].layer;
    mx += (tmx - mx) * 0.05;
    my += (tmy - my) * 0.05;
    trotY = tmx * 0.45 + Math.sin(T * 0.00018) * 0.14;
    trotX = -tmy * 0.28;
    ...
    for (let i = signals.length - 1; i >= 0; i--) {
      ...
      if (s.t >= 1) {
        const e = edges[s.e];
        const tg = s.dir === 1 ? nodes[e.b] : nodes[e.a];
        ...
        if (s.gen < 4) {
          if (s.dir === 1 && tg.layer < lastL) {
            for (const ei of tg.out)
              if (Math.random() < 0.6 && signals.length < (small ? 50 : 84))
                signals.push({
                  e: ei,
                  t: 0,
                  sp: s.sp * (0.9 + Math.random() * 0.2),
                  dir: 1,
                  gen: s.gen + 1,
                  col: C.mint,
                });
          } else if (s.dir === -1 && tg.layer > 0) {
            for (const ei of tg.inn)
              if (Math.random() < 0.5 && signals.length < (small ? 50 : 84))
                signals.push({
                  e: ei,
                  t: 0,
                  sp: s.sp * (0.9 + Math.random() * 0.2),
  ...
  function draw() {
    ...
    for (const s of stars) {
      ...
      if (s.bright) {
        const sp = glow(s.col, 26);
        ctx.globalAlpha = a * 0.9;
        ctx.drawImage(sp, px - 13, py - 13, 26, 26);
        ctx.globalAlpha = 1;
      }
      ctx.globalAlpha = a;
      ctx.fillStyle = s.col;
    ...
    for (const e of edges) {
      ...
      const al = (0.04 + act * 0.28) * Math.min(a.da, b.da);
      if (al < 0.012) continue;
      ctx.strokeStyle = `rgba(108,123,255,${al})`;
      ctx.beginPath();
    ...
    for (const n of drawOrder) {
      ...
      ctx.globalAlpha = (0.3 + 0.45 * n.da) * tw;
      ctx.drawImage(glow(C.sky, 64), n.sx - hr, n.sy - hr, hr * 2, hr * 2);
      if (n.act > 0.12) {
        ctx.globalAlpha = Math.min(1, n.act);
        const hr2 = baseR * (4 + n.act * 5);
        ctx.drawImage(
          glow(C.mint, 64),
          n.sx - hr2,
          n.sy - hr2,
      ...
      ctx.globalAlpha = n.da;
      ctx.fillStyle = n.act > 0.4 ? "#ffffff" : C.star;
      ctx.beginPath();
      ctx.arc(n.sx, n.sy, baseR * (0.55 + n.act * 0.4), 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.globalAlpha = 1;
    for (const s of signals) {
      ...
      const x = from.sx + (to.sx - from.sx) * s.t,
        y = from.sy + (to.sy - from.sy) * s.t;
      const t2 = Math.max(0, s.t - 0.16);
      const tx = from.sx + (to.sx - from.sx) * t2,
        ty = from.sy + (to.sy - from.sy) * t2;
      ...
      ctx.stroke();
      ctx.globalAlpha = 0.9;
      ctx.drawImage(glow(s.col, 30), x - 9, y - 9, 18, 18);
      ctx.globalAlpha = 1;
    ...
    for (const f of flares) {
      ...
      const gr = f.r * 0.5 + 8;
      ctx.drawImage(glow(f.col, 64), f.x - gr, f.y - gr, gr * 2, gr * 2);
    }
    ctx.globalAlpha = 1;
